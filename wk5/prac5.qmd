...
#读取伦敦行政区数据
Londonborough <- st_read(here::here("wk5",
                                    "statistical-gis-boundaries-london", 
                                    "ESRI", 
                                    "London_Borough_Excluding_MHW.shp")) %>%
  st_transform(., 27700) #将数据投影为 EPSG 27700 坐标系统（英国国家网格坐标系），以确保数据的坐标系统一致
...
Airbnb <- read_csv("listings.csv") %>%
  ...
  # 将数据转换为空间数据框（sf），其中 longitude 作为 x 坐标，latitude 作为 y 坐标，并指定坐标系为 WGS84 (EPSG:4326)
...
output<- data1%>%
  st_join(data2,.) %>%
  add_count(GSS_CODE, name="hotels_in_borough")  #基于每个行政区代码 (GSS_CODE) 统计酒店的数量，并将其存储在新列 hotels_in_borough 中
  return(output)
}

# use the function for hotels
Hotels <- Joinfun(OSM, Londonborough) #得到每个行政区中的酒店数量

# then for airbnb
# this is incorrect - change to airbnb2 to look at result
Airbnb <- Joinfun(Airbnb, Londonborough) #得到每个行政区中的airbnb数量
...
UK_outlinecrop <- UK_outline$geometry %>%
  st_crop(., newbb) #裁剪 UK_outline 数据，仅保留位于 newbb 范围内的区域

Hotels <- Hotels %>%
  #at the moment each hotel is a row for the borough
  #we just one one row that has number of airbnbs
  group_by(., GSS_CODE, NAME)%>%
  summarise(`Accomodation count` = unique(hotels_in_borough)) #在每个行政区内汇总住宿数量，将酒店和 Airbnb 记录为唯一值的统计
